...
from interpolate_var import interpolate_grid as interp_grid
from interpolate_var import interpolate_simple as interp_simple
from interpolate_var import interpolate_loop
import time
import os
import dill
...
class simulationExtended():
    ...
    def __init__(self,params):
        ...
        if self.params['load_pickle'] == True:
            ...
            self.ex = read_pickle('ext_boundarypoints')
        else:
    ...
    def simulate(self):
        ...
        for n in range(self.params['nsim']):  
            for i in range(1, self.t.shape[0]):
                ...
                shock2 = np.random.normal(0,1)*np.sqrt(1-self.corr**2)
                ...
                self.f_sim[i,n] = self.f_sim[i-1,n] + self.xi_u* (self.mu_avg-   self.f_sim[i-1,n] ) *self.dt + self.beta_f * (self.mu_u - self.f_sim[i-1,n]) * (self.f_sim[i-1,n] - self.mu_l)  * shock * np.sqrt(self.dt)
                ...
                if self.f_sim[i,n] < (self.mu_l):
                    self.f_sim[i,n] = 2 * (self.mu_l) - self.f_sim[i,n]
                if self.f_sim[i,n] >= self.mu_u:
                    self.f_sim[i,n] = 2 * self.mu_u - self.f_sim[i,n]
    ...
    def compute_statistics(self):
        ...
        self.stats =  pd.concat((self.stats, pd.Series({'No of crisis periods': self.Q_fn_crisis.shape[0], 'freq': int(1/self.dt), 'prob (dt freq)': self.prob,  'gammaE':self.gammaE,'gammaH':self.gammaH,'sigma':self.params['sigma'], 
...
if __name__ == '__main__':
    params={'rhoE': 0.06, 'rhoH': 0.03, 'aE': 0.11, 'aH': 0.03,
            ...
            'lambda_d':0, 'sigma':0.06, 'gammaE':5, 'gammaH':5, 'corr':1.0,
             'pi_u' : 0.01, 'f_u' : 0.2, 'f_l' : 0.1, 'f_avg': 0.15,
