...
class model_recursive():
    ...
    def __init__(self, params):
        ...
        self.Je = self.params['aE']**(-self.params['gammaE'])*self.z**(1-self.params['gammaE']); # value function of experts
        self.Jh = self.params['aE']**(-self.params['gammaH'])*(1-self.z)**(1-self.params['gammaH']); # value function of households
    ...
    def solve(self):
        ...
        for timeStep in range(self.maxIterations):
            ...
            advection = self.mu_z + (1-self.params['gammaE'])*self.params['sigma']*self.sig_za;
            linearTerm = (1-self.params['gammaE'])*(self.growthRate.reshape(-1,1) + self.params['rhoE']*(np.log(self.params['rhoE'])+np.log(self.z.reshape(-1,1)*self.q)) - self.params['gammaE']/2*self.params['sigma']**2) - np.log(self.Je.reshape(-1,1)); #PDE coefficient multiplying v(\z)
            ...
            advection = self.mu_z + (1-self.params['gammaH'])*self.params['sigma']*self.sig_za;
            linearTerm = (1-self.params['gammaH'])*(self.growthRate.reshape(-1,1) + self.params['rhoH']*(np.log(self.params['rhoH'])+np.log((1-self.z.reshape(-1,1))*self.q)) - self.params['gammaH']/2*self.params['sigma']**2) - np.log(self.Jh.reshape(-1,1)); #PDE coefficient multiplying v(\z)
...
if __name__ == '__main__':
    ...
    sim1 = model_recursive(params)
    sim1.solve()  
    params['lambda_d'] = 0.05
    sim2 = model_recursive(params)
    sim2.solve()
    params['zbar']
    ...
    params['lambda_d']=0.01
    params['zbar'] = 0.5
    sim3 = model_recursive(params)
    sim3.solve()
